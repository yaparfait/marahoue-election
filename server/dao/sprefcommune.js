...
async function createSprefCom(libsprefcom, spref, iddept) {   
    ...
    try {
        ...
        const insertQuery = "INSERT INTO spref_commune (idsprefcom, libsprefcom, spref, iddept) VALUES (?, ?, ?, ?)";
...
async function getSprefComByDepartement(iddept) {
    try {
        ...
        const result = await db.pool.query("SELECT * FROM spref_commune WHERE iddept=? ORDER BY spref", [iddept]);
        return result;
    } catch (err) {
        // Print errors
        console.log(err);
...
async function getSprefCommune() {
    try {
        // Use pool.query to get all contacts
        const sqlQuery = "SELECT idsprefcom,libsprefcom,spref,scom.iddept,libdept FROM spref_commune scom, departement dp WHERE scom.iddept=dp.iddept ORDER BY scom.iddept,spref";
